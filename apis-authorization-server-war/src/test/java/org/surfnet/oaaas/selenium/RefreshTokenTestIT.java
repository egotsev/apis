...
public class RefreshTokenTestIT extends SeleniumSupport {
	...
	public void refreshCode() throws Exception {
		/*
		 * First do a normal authorization and obtain a AccessToken (with
		 * refreshToken)
		 */
		...
		String url = String
				.format("%s/oauth2/authorize?response_type=%s&client_id=%s&scope=read&redirect_uri=%s",
						baseUrl(), responseType, clientId,
		...
		assertTrue(StringUtils
				.isNotBlank(accessTokenResponse.getRefreshToken()));
		...
		String postBody = String.format(
				"grant_type=%s&refresh_token=%s&state=%s",
				OAuth2Validator.GRANT_TYPE_REFRESH_TOKEN,
				accessTokenResponse.getRefreshToken(), "dummy");
		...
		AccessTokenResponse refreshTokenResponse = getMapper().readValue(
				responseAsString, AccessTokenResponse.class);

		assertTrue(StringUtils
				.isNotBlank(refreshTokenResponse.getAccessToken()));
		assertTrue(StringUtils.isNotBlank(refreshTokenResponse
				.getRefreshToken()));
		...
		assertNotSame(refreshTokenResponse.getAccessToken(),
				accessTokenResponse.getAccessToken());
		assertNotSame(refreshTokenResponse.getRefreshToken(),
				accessTokenResponse.getRefreshToken());
		assertEquals(refreshTokenResponse.getScope(),
				accessTokenResponse.getScope());
		assertEquals(refreshTokenResponse.getExpiresIn(),

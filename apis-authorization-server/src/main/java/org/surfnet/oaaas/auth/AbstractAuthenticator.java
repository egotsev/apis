...
public abstract class AbstractAuthenticator extends AbstractFilter {
	...
	public final void doFilter(ServletRequest request,
		...
		authenticate((HttpServletRequest) request,
				...
				getAuthStateValue(request), getReturnUri(request));
	}

	/**
	 * Implement this method to state whether the given request is a
	 * continuation that can be handled. This method will be called for every
	 * consecutive request after the initial one.<br />
	 * Returning true means that the request is part of an ongoing
	 * authentication.<br />
	 * Returning false indicates to the framework that the request is not known.<br />
	 * Typically this can be determined by the http method or one or more
	 * request parameters/attributes being present.
	 * 
	 * @param request
	 ...
	 * <p>
	 * assert that the user is authenticated. You can use the request and
	 * response for this. When not yet authenticated:
	 * </p>
	 * <ul>
	 * <li>use {@link #getAuthStateValue(javax.servlet.ServletRequest)} to
	 * pass-around for user agent communication</li>
	 * <li>use {@link #getReturnUri(javax.servlet.ServletRequest)} if you need
	 * to step out and return to the current location
	 * </ul>
	 * <p>
	 * When authenticated:
	 * </p>
	 ...
	 *            the authState nonce to set back on the {@link ServletRequest}
	 *            when done
	 * @param returnUri
	 *            the startpoint of the chain if you want to return from a form
	 *            or other (external) component
	 */
	public abstract void authenticate(HttpServletRequest request,
	 ...
	 */
	protected final void setPrincipal(ServletRequest request,

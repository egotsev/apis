...
public class OAuth2ValidatorImpl implements OAuth2Validator {
	...
	public ValidationResponse validate(AuthorizationRequest authorizationRequest) {
		try {
			...
			String redirectUri = determineRedirectUri(authorizationRequest,
					responseType, client);
	...
	protected List<String> determineScopes(
	...
	protected String determineRedirectUri(
			AuthorizationRequest authorizationRequest, String responseType,
			Client client) {
	...
	protected Client validateClient(AuthorizationRequest authorizationRequest) {
		...
		Client client = StringUtils.isBlank(clientId) ? null : clientRepository
				.findByClientId(clientId);
		if (client == null) {
			throw new ValidationResponseException(UNKNOWN_CLIENT_ID);
		}
		if (!client.isAllowedImplicitGrant()
				&& authorizationRequest.getResponseType().equals(
	...
	protected String validateResponseType(
			AuthorizationRequest authorizationRequest) {
		String responseType = authorizationRequest.getResponseType();
		if (StringUtils.isBlank(responseType)
				|| !RESPONSE_TYPES.contains(responseType)) {
	...
	protected void validateAccessTokenRequest(
			AccessTokenRequest accessTokenRequest) {
		if (accessTokenRequest.getGrantType().equals(
				GRANT_TYPE_CLIENT_CREDENTIALS)) {
			String clientId = accessTokenRequest.getClientId();
			Client client = StringUtils.isBlank(clientId) ? null

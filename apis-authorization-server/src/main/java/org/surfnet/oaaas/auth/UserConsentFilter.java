...
public class UserConsentFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		if (initialRequest(request)) {
			...
			/*
			 * Ok, the consentHandler wants to have control again (because he
			 * stepped out)
			 */
			userConsentHandler.doFilter(request, response, chain);
		}
	...
	private void storePrincipal(HttpServletRequest request,
	...
	private boolean initialRequest(HttpServletRequest request) {
		return (AuthenticatedPrincipal) request
				.getAttribute(AbstractAuthenticator.PRINCIPAL) != null;
	}
	 ...
	 */
	public void setUserConsentHandler(

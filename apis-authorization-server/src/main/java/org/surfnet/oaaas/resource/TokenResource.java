...
public class TokenResource {
	...
	private Response doProcess(HttpServletRequest request) {
		...
		if (authReq.getResponseType().equals(
				OAuth2Validator.IMPLICIT_GRANT_RESPONSE_TYPE)) {
	 ...
	 */
	private void processScopes(AuthorizationRequest authReq,
		...
		if (authReq.getClient().isSkipConsent()) {
			...
			String[] scopes = (String[]) request
					.getAttribute(AbstractUserConsentHandler.GRANTED_SCOPES);
	...
	private AccessToken createAccessToken(AuthorizationRequest request,
			boolean isImplicitGrant) {
		Client client = request.getClient();
		long expireDuration = client.getExpireDuration();
		long expires = (expireDuration == 0L ? 0L
				: (System.currentTimeMillis() + (1000 * expireDuration)));
		String refreshToken = (client.isUseRefreshTokens() && !isImplicitGrant) ? getTokenValue(true)
				: null;
		AuthenticatedPrincipal principal = request.getPrincipal();
		AccessToken token = new AccessToken(getTokenValue(false), principal,
				client, expires, request.getGrantedScopes(), refreshToken);
		return accessTokenRepository.save(token);
	}

	private AuthorizationRequest findAuthorizationRequest(
			HttpServletRequest request) {
		String authState = (String) request
				.getAttribute(AbstractAuthenticator.AUTH_STATE);
	...
	public Response token(@HeaderParam("Authorization") String authorization,
		...
		AccessTokenRequest accessTokenRequest = AccessTokenRequest
				.fromMultiValuedFormParameters(formParameters);
		UserPassCredentials credentials = getUserPassCredentials(authorization,
				accessTokenRequest);
		...
		try {
			if (GRANT_TYPE_AUTHORIZATION_CODE.equals(grantType)) {
				...
				// We have to construct a AuthenticatedPrincipal on-the-fly as
				// there is only key-secret authentication
				request.setPrincipal(new AuthenticatedPrincipal(request
		...
		AccessTokenResponse response = new AccessTokenResponse(
				token.getToken(), BEARER, request.getClient()
						.getExpireDuration(), token.getRefreshToken(),
	...
	private AuthorizationRequest authorizationCodeToken(
			AccessTokenRequest accessTokenRequest) {
		AuthorizationRequest authReq = authorizationRequestRepository
		...
		if (!authReq.getRedirectUri().equalsIgnoreCase(uri)) {
			throw new ValidationResponseException(
					ValidationResponse.REDIRECT_URI_DIFFERENT);
		}
		authorizationRequestRepository.delete(authReq);
		return authReq;
	}

	private AuthorizationRequest refreshTokenToken(
			AccessTokenRequest accessTokenRequest) {
		AccessToken accessToken = accessTokenRepository
				.findByRefreshToken(accessTokenRequest.getRefreshToken());
		if (accessToken == null) {
			throw new ValidationResponseException(
					ValidationResponse.INVALID_GRANT_REFRESH_TOKEN);
		}
	...
	private Response sendErrorResponse(String error, String description) {
		return Response.status(Status.BAD_REQUEST)
				.entity(new ErrorResponse(error, description)).build();
	}
	...
	private Response sendImplicitGrantResponse(AuthorizationRequest authReq,
		...
		String fragment = String.format(
				"access_token=%s&token_type=bearer&expires_in=%s&scope=%s"
						+ appendStateParameter(authReq),
				accessToken.getToken(), accessToken.getExpires(),
				StringUtils.join(authReq.getGrantedScopes(), ','));
		if (authReq.getClient().isIncludePrincipal()) {
			fragment += String.format("&principal=%s", authReq.getPrincipal()
					.getDisplayName());
		}
		return Response.seeOther(
				UriBuilder.fromUri(uri).fragment(fragment).build()).build();
	 ...
	 */
	public void setAccessTokenRepository(

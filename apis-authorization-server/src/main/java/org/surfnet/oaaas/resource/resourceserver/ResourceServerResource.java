...
public class ResourceServerResource extends AbstractResource {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ResourceServerResource.class);
	 ...
	 * or in case of an adminPrincipal we return all resource servers.
	 */
	@GET
	...
	public Response stats(@Context HttpServletRequest request) {
		Iterable<ResourceServer> resourceServers = this
				.getAllResourceServers(request);
		...
		for (ResourceServer resourceServer : resourceServers) {
			...
			for (Client client : clients) {
				clientStats.add(new StatisticsResponse.ClientStat(client
						.getName(), client.getDescription(),
						accessTokenRepository
								.countByUniqueResourceOwnerIdAndClientId(client
										.getId())));
			}
			resourceServerStats.add(new StatisticsResponse.ResourceServerStat(
					resourceServer.getName(), resourceServer.getDescription(),
					clientStats));
		}
	...
	public Response put(@Context HttpServletRequest request,
		...
		try {
			// we run transactional modus, so any constraint violations only
			// occur after the commit of the transaction (to late...)
			validate(newOne);
		...
		LOG.debug("New resourceServer has been saved: {}. ",
				resourceServerSaved);

		final URI uri = UriBuilder.fromPath("{resourceServerId}.json").build(
				resourceServerSaved.getId());
	...
	public Response post(@Valid final ResourceServer resourceServer,
		...
		pruneClientScopes(resourceServer.getScopes(),
				persistedResourceServer.getScopes(),
				persistedResourceServer.getClients());
		LOG.debug(
				"About to update existing resourceServer {} with new properties: {}",
	...
	protected void pruneClientScopes(final List<String> newScopes,
			List<String> oldScopes, Set<Client> clients) {
		if (!newScopes.containsAll(oldScopes)) {
			...
			LOG.info(
					"Resource server has updated scopes. Will remove all outdated scopes from clients: {}",
					outdatedScopes);

			for (Client c : clients) {
				final List<String> clientScopes = c.getScopes();
				if (CollectionUtils.containsAny(clientScopes, outdatedScopes)) {
					ArrayList<String> prunedScopes = new ArrayList<String>(
							subtract(clientScopes, outdatedScopes));
					LOG.info(
							"Client scopes of client {} were: {}. After pruning (because resourceServer has new scopes): {}",
							new Object[] { c.getClientId(), c.getScopes(),
									prunedScopes });
	...
	private ResourceServer getResourceServer(HttpServletRequest request, Long id) {
		...
		LOG.debug("About to return one resourceServer with id {}: {}", id,
				resourceServer);
		return resourceServer;
	}

	private List<ResourceServer> getAllResourceServers(
			HttpServletRequest request) {
		List<ResourceServer> resourceServers;
		if (isAdminPrincipal(request)) {
			resourceServers = addAll(resourceServerRepository.findAll()
					.iterator());
			LOG.debug(

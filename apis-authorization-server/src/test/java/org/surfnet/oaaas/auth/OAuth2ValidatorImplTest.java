...
public class OAuth2ValidatorImplTest {
	...
	public void before() {
		...
		when(clientRepository.findByClientId(client.getClientId())).thenReturn(
				client);
	...
	public void determineRedirectUri() {
		...
		validator.determineRedirectUri(request, "code",
	...
	public void implicitGrantNoRedirectGivenShouldUseDefault() {
		...
		try {
			final String determinedUri = validator.determineRedirectUri(
					request, OAuth2ValidatorImpl.IMPLICIT_GRANT_RESPONSE_TYPE,
					client);
			fail();
	...
	public void testClientCredentialsTokenRequest() {
		...
		accessTokenRequest
				.setGrantType(OAuth2Validator.GRANT_TYPE_CLIENT_CREDENTIALS);
		accessTokenRequest.setClientId(client.getClientId());
		ValidationResponse response = validator.validate(accessTokenRequest);
		assertEquals(ValidationResponse.CLIENT_CREDENTIALS_NOT_PERMITTED,
				response);
	...
	private Client createClient(String clientId) {
		...
		client.setRedirectUris(Arrays.asList("http://gothere.nl",
				"http://gohere.nl"));

...
public class ClientController {
	...
	public String redirect(ModelMap modelMap, HttpServletRequest request,
		...
		String auth = "Basic ".concat(new String(Base64.encodeBase64(settings
				.getOauthKey().concat(":").concat(settings.getOauthSecret())
				.getBytes())));
		Builder builder = client.resource(settings.getAccessTokenEndPoint())
		...
		modelMap.put(
				"requestInfo",
				"Method: POST".concat(BR).concat("URL: ")
						.concat(settings.getAccessTokenEndPoint()).concat(BR)
						.concat("Headers: ").concat(headers.toString())
						.concat(BR).concat("Body: ")
	...
	public String step3(ModelMap modelMap,
		...
		Builder builder = client
				.resource(settings.getRequestURL())
				.header(AUTHORIZATION,
						"bearer ".concat(settings.getAccessToken()))
				.type(MediaType.APPLICATION_FORM_URLENCODED_TYPE);
		OutBoundHeaders headers = getHeadersCopy(builder);
		...
		settings.setStep("step3");
		modelMap.put(SETTINGS, settings);
		modelMap.put("requestInfo", "Method: GET".concat(BR).concat("URL: ")
				.concat(settings.getRequestURL()).concat(BR)
				.concat("Headers: ").concat(headers.toString()));
		addResponseInfo(modelMap, clientResponse);
		modelMap.put("rawResponseInfo", json);
		return "oauth-client";
	...
	private void addResponseInfo(ModelMap modelMap,
		...
		modelMap.put(
				"responseInfo",
				"Status: ".concat(String.valueOf(clientResponse.getStatus())
						.concat(BR).concat("Headers:")
	...
	/**
	 * See /apis-authorization-server/src/main/resources/db/migration/hsqldb/
	 * V1__auth-server-admin.sql
	 */

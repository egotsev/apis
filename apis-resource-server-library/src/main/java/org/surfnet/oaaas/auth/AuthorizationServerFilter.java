...
public class AuthorizationServerFilter implements Filter {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(AuthorizationServerFilter.class);
	...
	/*
	 * By default we respond to preflight CORS requests and have a lenient
	 * policy as we are secured by OAuth2
	 */
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		/*
		 * First check on the presence of a init-param where to look for the
		 * properties to support multiple resource servers in the same war. Then
		 * look for second best apis-resource-server.properties file, then try
		 * to use the filter config if parameters are present. If this also
		...
		String propertiesFile = filterConfig
				.getInitParameter("apis-resource-server.properties.file");
		...
		if (res != null && res.exists()) {
			...
			try {
				...
				throw new RuntimeException(
						"Error in reading the apis-resource-server.properties file",
						e);
			}
			resourceServerKey = prop
					.getProperty("adminService.resourceServerKey");
			resourceServerSecret = prop
					.getProperty("adminService.resourceServerSecret");
			authorizationServerUrl = prop
					.getProperty("adminService.tokenVerificationUrl");
			cacheEnabled = Boolean.valueOf(prop
					.getProperty("adminService.cacheEnabled"));
			String allowCorsRequestsProperty = prop
					.getProperty("adminService.allowCorsRequests");
			if (StringUtils.isNotEmpty(allowCorsRequestsProperty)) {
				allowCorsRequests = Boolean.valueOf(allowCorsRequestsProperty);
			}
			String typeInformationIsIncludedProperty = prop
					.getProperty("adminService.jsonTypeInfoIncluded");
			if (StringUtils.isNotEmpty(typeInformationIsIncludedProperty)) {
				typeInformationIsIncluded = Boolean
						.valueOf(typeInformationIsIncludedProperty);
			}
		} else if (filterConfig.getInitParameter("resource-server-key") != null) {
			resourceServerKey = filterConfig
					.getInitParameter("resource-server-key");
			resourceServerSecret = filterConfig
					.getInitParameter("resource-server-secret");
			authorizationServerUrl = filterConfig
					.getInitParameter("authorization-server-url");
			typeInformationIsIncluded = Boolean.valueOf(filterConfig
					.getInitParameter("type-information-is-included"));
		}
		Assert.hasText(resourceServerKey, "Must provide a resource server key");
		Assert.hasText(resourceServerSecret,
				"Must provide a resource server secret");
		Assert.hasText(authorizationServerUrl,
				"Must provide a authorization server url");

		this.authorizationValue = new String(
				Base64.encodeBase64(resourceServerKey.concat(":")
	...
	protected VerifyTokenResponse getVerifyTokenResponse(String accessToken) {
		...
		if (verifyTokenResponse == null) {
			...
			try {
				...
				LOG.debug("Got verify token response (status: {}): '{}'",
						statusCode, responseString);
				if (statusCode == HttpServletResponse.SC_OK) {
					verifyTokenResponse = objectMapper.readValue(
							responseString, VerifyTokenResponse.class);
				}
			} catch (Exception e) {
				LOG.error(
						"Exception in reading result from AuthorizationServer",
						e);
	...
	protected void sendError(HttpServletResponse response, int statusCode,
			String reason) {
		LOG.warn(
				"No valid access-token on request. Will respond with error response: {} {}",
				statusCode, reason);
		try {
	 ...
	 */
	protected boolean handledCorsPreflightRequest(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		if (!this.allowCorsRequests
				|| StringUtils.isBlank(request.getHeader("Origin"))) {
			return false;
		}
		/*

 ...
 * Please carefully monitor the performance / cache hit-ratio in production as
 * the maxSize in combination with the expireTimeSeconds is important. If the
 * maxSize is too small and the expireTimeSeconds to low it will result in a
 * cache that with each addition will try to make space (e.g. effectively only
 * removing the oldest entry each time).
 */
public class TokenResponseCacheImpl implements TokenResponseCache {
	...
	private void invariant() {
		...
		Assert.isTrue(expireTime < ((1000 * 60 * 60 * 24) + 1),
				"Maximal expireTime is one day");
		Assert.isTrue(expireTime > 0,
				"ExpireTimeMilliseconds must be greater then 0");
	}
	...
	public void storeVerifyToken(String accessToken,
		...
		if (accessToken != null && tokenResponse != null) {
			...
			cache.put(accessToken,
					new CacheEntry(tokenResponse, System.currentTimeMillis()
							+ expireTime));
		}
	}

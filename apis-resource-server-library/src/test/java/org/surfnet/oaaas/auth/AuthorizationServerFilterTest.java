...
/**
 * {@link Test} that uses a dummy http server to org.surfnet.oaaas.conext.mock
 * return values from the authorization server.
 * 
 */
public class AuthorizationServerFilterTest extends AbstractMockHttpServerTest {

	private static final ObjectMapper mapper = new ObjectMapperProvider()
	...
	public void before() throws ServletException {
		...
		filterConfig.addInitParameter("resource-server-key",
				"org.surfnet.oaaas.conext.mock-server-name");
		filterConfig.addInitParameter("resource-server-secret", UUID
				.randomUUID().toString());
	...
	public void testDoFilterHappyFlow() throws IOException, ServletException {
		...
		VerifyTokenResponse recorderdResponse = new VerifyTokenResponse(
				"org.surfnet.oaaas.conext.mock-client",
				Collections.singletonList("read"),
				new AuthenticatedPrincipal("john.doe", Arrays.asList("user",
						"admin"), attributes), 0L);
		...
		assertEquals("value", response.getPrincipal().getAttributes()
				.get("key"));
		assertEquals("*",
				((MockHttpServletResponse) chain.getResponse())
						.getHeader("Access-Control-Allow-Origin"));

		/*
		 * Also test the cache by repeating the call and setting the expected
		 * result to null (which would cause an exception in
		 * MockHandler#invariant if the cache does not kick in)
		 */
	 ...
	 */
	@Test
	public void testDoFilterWrongAccessToken() throws IOException,
			ServletException {
	...
	public void testCorsHeadersPreflight() throws IOException, ServletException {
		...
		request.addHeader("Access-Control-Request-Headers",
				HttpHeaders.AUTHORIZATION + ", X-Custom-Header");
		request.setMethod("OPTIONS");

		MockHttpServletResponse response = new MockHttpServletResponse();
		// null FilterChain as chain.filter to proceed the flow is not called
		// with preflight
	...
	private MockFilterChain doCallFilter(VerifyTokenResponse recorderdResponse)
	...
	private MockFilterChain doCallFilter(VerifyTokenResponse recorderdResponse,
			...
			ServletException {
		return doCallFilter(new Resource[] { new ByteArrayResource(mapper
				.writeValueAsString(recorderdResponse).getBytes(), "json") },
				response);
	}

	private MockFilterChain doCallFilter(Resource[] resource,
			MockHttpServletResponse response) throws IOException,
			ServletException {
